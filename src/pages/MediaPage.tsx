...
const stages = [
  { value: "idea", label: "待开始" },
  { value: "planning", label: "已计划" },
  { value: "producing", label: "进行中" },
...
export function MediaPage() {
  ...
  return (
    <div>
      <PageHeader icon={<ModuleArtwork module="media" />} eyebrow="工作流程" title="工作进度" description="从计划、执行到完成，把工作放在真正的流程里。" actions={<Button onClick={() => setEditing({ stage: "idea" })}><Plus size={18} />记录内容</Button>} />
      ...
      {data.mediaContents.length === 0 ? <EmptyState title="还没有工作记录" description="先记录一个标题，其他信息可以以后再补。" action={<Button variant="secondary" onClick={() => setEditing({ stage: "idea" })}>记录工作</Button>} /> : null}
