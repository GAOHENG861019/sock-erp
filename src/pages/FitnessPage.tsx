...
type RawMaterial = { id: string; name: string; spec: string; weight: number; amount: number; photo?: string };
...
export function FitnessPage() {
  ...
  return (
    <div>
      <PageHeader icon={<ModuleArtwork module="fitness" />} eyebrow="采购与供应商" title="原材料采购" description="记录原材料采购明细，自动合计重量和金额。" actions={<Button onClick={() => setRawDialog({ id: "", name: "", spec: "", weight: 0, amount: 0 })}><Plus size={17} />添加原材料</Button>} />
      <div className="overview-strip" style={{ marginBottom: 16 }}>
        <div><span>原材料种类</span><strong>{rawMaterials.length}</strong></div>
        <div><span>总重量</span><strong>{rawWeight.toFixed(2)}<small> kg</small></strong></div>
        <div><span>采购总额</span><strong>¥{rawTotal.toFixed(2)}</strong></div>
      </div>
      <Section title="原材料清单" description="添加名称、规格、重量和金额，可上传照片">
        {rawMaterials.length ? <><table className="prod-table"><thead><tr><th>照片</th><th>名称</th><th>规格</th><th>重量(kg)</th><th>金额</th><th>操作</th></tr></thead><tbody>{rawMaterials.map((item) => <tr key={item.id}><td>{item.photo ? <img src={item.photo} alt={item.name} style={{ width: 48, height: 48, objectFit: "cover", borderRadius: 6 }} /> : <span style={{ color: "#999", fontSize: 12 }}>无</span>}</td><td>{item.name}</td><td>{item.spec || "-"}</td><td>{item.weight}</td><td>¥{Number(item.amount).toFixed(2)}</td><td><button className="icon-button danger-text" title="删除" onClick={() => setRawMaterials((prev) => prev.filter((r) => r.id !== item.id))}><Trash size={16} /></button></td></tr>)}</tbody></table><div className="prod-summary prod-grand"><span><Calculator size={18} />原材料总额度：<strong>¥{rawTotal.toFixed(2)}</strong></span></div></> : <EmptyState title="还没有原材料" description="点击添加原材料，记录名称、规格、重量和金额。" action={<Button variant="secondary" onClick={() => setRawDialog({ id: "", name: "", spec: "", weight: 0, amount: 0 })}>添加第一个原材料</Button>} />}
      </Section>
...
function RawMaterialDialog({ open, onClose, onSave }: { open: RawMaterial | null; onClose: () => void; onSave: (item: RawMaterial) => void }) {
  const [form, setForm] = useState<RawMaterial>({ id: "", name: "", spec: "", weight: 0, amount: 0 });
  ...
  return (
    <Modal open title={open.id ? "编辑原材料" : "添加原材料"} description="填写名称、规格、重量和金额，可上传照片" onClose={onClose}>
      <div className="form-grid">
        ...
        <label className="form-field"><span>重量(kg)</span><input type="number" step="0.01" value={form.weight || ""} onChange={(e) => setForm({ ...form, weight: Number(e.target.value) || 0 })} placeholder="kg" /></label>
        <label className="form-field"><span>金额(元)</span><input type="number" step="0.01" value={form.amount || ""} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) || 0 })} placeholder="元" /></label>
      </div>
