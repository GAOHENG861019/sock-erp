...
export function ConsultingPage() {
  ...
  return (
    <div>
      <PageHeader icon={<ModuleArtwork module="consulting" />} eyebrow="客户关系管理" title="客户中心" description="管理客户档案、联系方式、项目和沟通记录。" actions={<><Button variant="secondary" onClick={() => setDialog({ type: "client" })}><Users size={17} />添加客户</Button>{clientId ? <Button onClick={() => setDialog({ type: "project" })}><Plus size={17} />新建项目</Button> : null}{client ? <Button variant="ghost" className="danger-text" onClick={() => setDeleteClientOpen(true)}><Trash size={16} />删除客户</Button> : null}</>} />
          ...
          {project ? <>
            ...
            <div className="consult-grid">
              <Section title="交付物" description="明确截止日期和交付状态">
                {deliverables.length ? <div className="check-list">{deliverables.map((item) => <article key={item.id}><button className="complete-control" disabled={item.status === "done"} onClick={() => void run(() => api.update("consultingDeliverables", item.id, { status: "done" }))}>{item.status === "done" ? <Check size={14} /> : null}</button><div><strong>{item.name}</strong><small>{item.due_date ? `截止 ${formatDate(item.due_date)}` : "没有截止日期"}</small></div><button className="icon-button" title="加入本月总览" onClick={() => void run(() => api.create("planItems", { title: item.name, plan_date: localDate(), source_module: "consulting", source_entity_type: "consulting_deliverable", source_entity_id: item.id, priority: "high" }))}><CalendarPlus size={17} /></button></article>)}</div> : <p className="quiet-line">暂无交付物。</p>}
              </Section>
              <Section title="待跟进" description="到了日期会出现在首页提醒中">
                {followups.length ? <div className="check-list">{followups.map((item) => <article key={item.id}><button className="complete-control" disabled={item.status === "done"} onClick={() => void run(() => api.update("consultingFollowups", item.id, { status: "done", completed_at: new Date().toISOString() }))}>{item.status === "done" ? <Check size={14} /> : null}</button><div><strong>{item.content}</strong><small>{formatDateTime(item.followup_at)}</small></div><button className="icon-button" title="加入本月总览" onClick={() => void run(() => api.create("planItems", { title: item.content, plan_date: localDate(), source_module: "consulting", source_entity_type: "consulting_followup", source_entity_id: item.id, priority: "medium" }))}><CalendarPlus size={17} /></button></article>)}</div> : <p className="quiet-line">暂无跟进安排。</p>}
              </Section>
            </div>
...
function ConsultingDialog({ dialog, close, clientId, project, run }: any) {
  ...
  const map: Record<string, { title: string; collection: any; fields: FieldDefinition[]; defaults: Record<string, any> }> = {
    client: { title: "添加客户", collection: "clients", fields: [{ name: "name", label: "客户名称", required: true }, { name: "phone", label: "联系电话" }, { name: "address", label: "地址" }, { name: "notes", label: "备注", type: "textarea" }], defaults: {} },
